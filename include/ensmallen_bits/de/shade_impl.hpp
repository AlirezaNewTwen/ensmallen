...
inline SHADE::SHADE(const size_t populationSize,
    ...
    const double differentialWeight,
    const double tolerance,
    ens::ConstraintsNT<MatType> myConstraints) :
    populationSize(populationSize),
    maxGenerations(maxGenerations),
    crossoverRate(crossoverRate),
    differentialWeight(differentialWeight),
                                         ...
                                         CallbackTypes&&... callbacks)
{
  ...
  constraints.enforceConstraints(iterate);
  ...
  {
      population[i] = constraints.get_lower_bound() +
        arma::randu(problem_size) %
        (constraints.get_upper_bound() - constraints.get_lower_bound());

  	constraints.enforceConstraints(population[i]);

    if (i == 0)
    {
        population[0] = 0.5 * (constraints.get_upper_bound() + constraints.get_lower_bound());
    }
