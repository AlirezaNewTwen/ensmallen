                                         ...
                                         CallbackTypes&&... callbacks)
{
  ...
  int memory_size = iterate.n_rows;
  ...
  arma::vec archive_funvalues = arma::zeros<arma::vec>(0);
  ...
  for (size_t gen = 0; gen < maxGenerations && !terminate; gen++)
  {
      ...
      //arma::vec<bool> goodCR = arma::zeros<arma::vec<bool>>(populationSize);

      
      //goodF.set_size(populationSize);


      // Find positions where mu_cr == -1
      arma::uvec term_pos = arma::find(mu_cr == -1);
      // Set elements of cr to 0 where mu_cr == -1
      crS.elem(term_pos).fill(0);
      // Clip cr values to [0, 1]
      crS = arma::clamp(crS, 0.0, 1.0);

      // Generate scaling factor
      arma::vec sf = mu_sf + 0.1 * arma::tan(arma::datum::pi * (arma::randu<arma::vec>(pop_size) - 0.5));
      // Find positions where sf is less than or equal to 0
      ...
      // Calculate the number of solutions to choose (at least two best solutions)
      ...
      //randindex = arma::max(randindex, 0);


      std::vector<BaseMatType> pbest;
      pbest.resize(pop_size);
	  arma::vec pbestFitness = arma::zeros<arma::vec>(pop_size);
      for (std::size_t i = 0; i < pop_size; ++i) {
          std::size_t index = randindex[i];
          if (index < sorted_index.size()) {
              pbest[i] = population[sorted_index[index]];
              pbestFitness[i] = fitnessValues[sorted_index[index]];
          }
      }
    ...
    std::cout << "updateArchiveupdateArchiveupdateArchiveupdateArchiveupdateArchiveupdateArchiveupdateArchiveupdateArchiveupdateArchiveupdateArchiveupdateArchiveupdateArchiveupdateArchiveupdateArchiveupdateArchiveupdateArchive"<< std::endl;
    std::cout << "      [fitness, I] = min([fitness, children_fitness], [], 2)fnalksjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjj" << std::endl;
